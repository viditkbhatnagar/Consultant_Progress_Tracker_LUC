...
- **Schedule upload** (`POST /api/institute/timetable/import`, Timetable → "Upload Schedule", `client/src/components/institute/ImportScheduleDialog.js`): bulk-import a workbook with **one sheet per teacher** (sheet name = teacher). `dryRun=true` previews (parses + reports, writes nothing); `dryRun=false` applies. Parser: `server/services/institute/scheduleParser.js` — pure (Buffer in, `{teachers, timetable, warnings}` out), shared with `scripts/importInstituteFromExcel.js`.
- **Applying REPLACES only the teachers present in the file**, so one teacher's upload can never wipe another's schedule. Within a teacher the order is capture-old-ids → `insertMany(new)` → `deleteMany(old)`: a failure leaves duplicates (visible, fixable) rather than an emptied schedule. There is **no transaction** — instead every row is validated *before* the first write, so the realistic failure (a bad row in the last sheet) rejects the whole upload with a 400 and zero writes.
- **Merged Day cells are the norm and must keep working.** Excel merges the Day column down each day's block, so continuation rows arrive blank; the parser forward-fills the last day. Without that it silently dropped most of a normal schedule and the replace then deleted those real sessions. A **non-blank** day cell that can't be parsed is reported in `warnings` (with the Excel row number) — never dropped quietly. Abbreviations (`Mon`, `Thurs`) and date-formatted day cells are handled. Specs: `server/tests/institute/scheduleImport.test.js`.
- Subjects from the sheet run through `canonicalizeSubject()` so an import can't reintroduce the duplicate spellings the canonical-subjects work removed. Teachers are matched case-insensitively (sheets whose names normalize alike collapse to one teacher), created if new, and re-activated if previously deactivated; their `subjects` are unioned so manual additions survive a re-import.

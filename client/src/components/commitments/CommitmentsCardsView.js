...
const CommitmentCard = ({ row, onOpen }) => {
    const palette = getStagePalette(row.leadStage);
    const { day, month } = splitDate(displayDate(row));
    ...
    return (
        ...
        >
            ...
            >
                ...
                <StatusPill status={row.leadStage} size="sm" />
            </Box>
            ...
            >
                {row.commitmentMade || <Box component="span" sx={{ color: 'var(--t-text-faint)' }}>No commitment description</Box>}
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 1.25 }}>
                <Typography
                    sx={{
                        fontSize: 11,
                        fontWeight: 600,
                        color: achievement === 100 ? 'var(--t-success-text)' : 'var(--t-text-3)',
                    }}
                >
                    {row.conversionProbability ?? 0}% · {row.meetingsDone || 0} meetings · {achievement}% ach.
                </Typography>
...
const CommitmentsCardsView = ({ rows, loading, onOpen }) => {
    if (loading) {
        return (
            <Box
                sx={{
    ...
    return (
        ...
        >
            <BoardHorizontalScroller variant="embedded">
                ...
                >
                    {rows.map((r) => (
                        <CommitmentCard key={r._id} row={r} onOpen={onOpen} />
                    ))}
                </Box>
...
export default CommitmentsCardsView;

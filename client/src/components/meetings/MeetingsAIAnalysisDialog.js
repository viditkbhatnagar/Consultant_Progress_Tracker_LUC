...
// Very lightweight markdown → HTML renderer for the AI response.
// The prompt returns a small, known shape (h2/h3/bullets/bold). No
// external dep needed.
...
const MeetingsAIAnalysisDialog = ({ open, onClose, loading, analysis, onRefresh, error }) => {
    return (
        <Dialog
            open={open}
            onClose={onClose}
            ...
            <DialogContent sx={{ padding: '18px 20px', backgroundColor: 'var(--t-surface)' }}>
                {loading ? (
                    ...
                    >
                        ...
                        <Typography sx={{ fontSize: 12.5, color: 'var(--t-text-muted)' }}>
                            Crunching the numbers…
                        </Typography>
                    </Box>
                ) : error ? (
                    <Box sx={{ py: 4 }}>
                        <Typography sx={{ fontSize: 13, color: 'var(--t-danger-text)', mb: 2 }}>
                            {error}
                        </Typography>
...
export default MeetingsAIAnalysisDialog;
